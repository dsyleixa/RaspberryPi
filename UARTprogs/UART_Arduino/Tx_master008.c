...
void displayvalues(int line, uint8_t array[]) {
  ...
  for(cnt=0; cnt<16; ++cnt) {
    sprintf(sbuf, "%3d ", array[cnt+16]);      // print on TFT
    lcdprintxy(cnt*3*_fontsize_, line-20, sbuf);
    //printf(sbuf);
  }  
 
  for(cnt=0; cnt<16; ++cnt) {
    sprintf(sbuf, "%3d ", array[cnt+32]);      // print on TFT
    lcdprintxy(cnt*3*_fontsize_, line-40, sbuf);
    //printf(sbuf);
  }  
  for(cnt=0; cnt<16; ++cnt) {
    sprintf(sbuf, "%3d ", array[cnt+48]);      // print on TFT
...
void loop()
{
  ...
  if( resOK ) {                         // byte 0 == syncbyte ?
    ...
    lcdprintxy(0,_scrheight_-220, "recvbuf");
    displayvalues(_scrheight_-240, cbuf);
