...
uint8_t  currbuf[MSGSIZE];
uint8_t  dispbufs[MSGSIZE], dispbufr[MSGSIZE];
...
void* thread0display (void* ) {

    printf("init graph..."); printf("\n");
    Initgraph();
            
    while(sem_threadrun) {
	   cls(); 
       displayvalues(_scrheight_-40, dispbufs);
       delay(10);
       displayvalues(_scrheight_-260, dispbufr);
       delay(40);	
...
void loop()
{
  ...
  memcpy(dispbufs, sendbuf, sizeof(sendbuf));
  ...
  memset(currbuf, 0, sizeof(currbuf));
   
  resOK = receive ( currbuf, 10000, &cnt);
 
  if( resOK ) {                         // byte 0 == syncbyte ?
     cnt=0;
     memcpy(recvbuf, currbuf, sizeof(currbuf));
     memcpy(dispbufr, currbuf, sizeof(currbuf));
     
     // displayvalues(_scrheight_-260, cbuf);
     
     decyphdval(recvbuf);
     
     chk=(byte)calcchecksum(currbuf);    

     // test: change values to send back!
...
int main() {
    ...
    pthread_create(&thread0, NULL, thread0display, NULL);
