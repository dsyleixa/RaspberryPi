...
int    K,
       RootEval,
       Rootep;              // enpasaant.PieceType+square
       R,
       HashKeyLo,
       HashKeyHi;
                       ...
                       /* prev=prev.dest; HashKeyLo,HashKeyHi=hashkeys; return e*/
{
   int  j,
        ...
        C,
        s;
   uint8_t   t,                     // Fix B: board indices must be unsigned   <<<<<<<<<<<<<<<<
        PieceType,
        Piece,
        x,
        y,
   ...
   // Fix A: correct hash index precedence
   struct HTab *a = HashTab+( (HashKeyLo+turn*epSqr) & (HASHSIZE-1) );   //  lookup pos. in hash table
   ...
   while( d++ <  Depth || d<3
         || prev&K == INF
         && ( N<1000000L & d<98                  /* root: deepen upto time   */
            || (K=X, L=Y&~M, d=3)
       )
     )                                          /* time's up: go do best    */
   {
      x=B=X;                                       /* start scan at prev. best */
      h=Y&S;                                       /* request try noncastl. 1st*/m=-P<l | R>35 ? d>2 ? -I : e : -P;
      ...
      do
      {
         ...
         if(Piece & turn)                                   /*  own piece (inefficient!)*/
         {
...
labelA:                                        /* resume normal after best */
               ...
               {                                       /* y traverses ray, or:     */
                  ...
                  if(t&turn|PieceType<3&!(y-x&7)-!t)break;            /* capt. own, bad pawn mode */
                  ...
                  if(d-!t>1)                               /* remaining depth          */
                  {
                     ...
                     if(!(G&M))board[SkipSqr]=turn+6,v+=50;               /* castling: put R & Eval  */
                     ...
                     HashKeyLo+=KAB(y+0,board[y])-KAB(x+0,Piece)-KAB(H+0,t);
                     HashKeyHi+=KAB(y+8,board[y])-KAB(x+8,Piece)-KAB(H+8,t)+G -S;  /* update hash key          */
                  ...
                  if (
                    ...
                    PieceType>2 & (PieceType-4|j-7 ||                /* no P & no lateral K move,*/
                       board[G=x+3 ^ StepVec>>1&7]-turn-6    /* no virgin R in corner G, */
                      || board[G^1] | board[G^2] )          /* no 2 empty sq. next to R */
                    )
                  {
                     t+=PieceType<5;                             /* fake capt. for nonsliding*/
                  }
