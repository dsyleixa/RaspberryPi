...
int main() {
    ...
    pthread_create(&thread0, NULL, thread0Name, NULL);    // low priority task: screen output
    param.sched_priority = 20;
    pthread_setschedparam(thread0, SCHED_RR, &param);
   
    pthread_create(&thread1, NULL, thread1Name, NULL);    // low priority: keyboard monitoring (stop program)
    param.sched_priority = 20;
    pthread_setschedparam(thread1, SCHED_RR, &param);
   
    pthread_create(&thread2, NULL, thread2Name, NULL);    // medium  priority: motor control
    param.sched_priority = 40;
    pthread_setschedparam(thread2, SCHED_RR, &param);
   
    pthread_create(&thread3, NULL, thread3Name, NULL);    // highest priority: encoder reading     
    param.sched_priority = 80;
    pthread_setschedparam(thread3, SCHED_FIFO, &param);
   
    pthread_create(&thread4, NULL, thread4Name, NULL);    // medium priority:  UART comm   <<< test !!
    param.sched_priority = 40;
    pthread_setschedparam(thread4, SCHED_RR, &param);
 
    pthread_create(&thread5, NULL, thread5Name, NULL);    // medium priority: navigation
    param.sched_priority = 40;
    pthread_setschedparam(thread5, SCHED_RR, &param);
    
    pthread_create(&thread6, NULL, thread6Name, NULL);    // medium priority: RC heartbeat
    param.sched_priority = 40;
